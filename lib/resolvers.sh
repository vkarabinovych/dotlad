...
resolver_known() {
    ...
    resolver_method "$1" apply; apply="$RESOLVER_METHOD"
    resolver_method "$1" render; render="$RESOLVER_METHOD"
    resolver_method "$1" equal; equal="$RESOLVER_METHOD"
...
resolver_supports() {  # <resolver> <file|directory>
    local fn
    resolver_method "$1" supports; fn="$RESOLVER_METHOD"
    if declare -F "$fn" >/dev/null 2>&1; then "$fn" "$2"
    else [[ "$2" == file ]]; fi
}
...
resolver_render() {  # <resolver> <repo-source> <live-destination>
    local fn
    resolver_method "$1" render; fn="$RESOLVER_METHOD"
    shift
    "$fn" "$@"
}
...
resolver_equal() {  # <resolver> <repo-source> <live-destination>
    local fn
    resolver_method "$1" equal; fn="$RESOLVER_METHOD"
    shift
    "$fn" "$@"
}
...
resolver_check() {  # <resolver> <repo-source> <live-destination>
    ...
    resolver_method "$resolver" check; fn="$RESOLVER_METHOD"
    if declare -F "$fn" >/dev/null 2>&1; then "$fn" "$src" "$dest"; return; fi
    resolver_has_method "$resolver" render || return 0
    if [[ -d "$dest" && ! -L "$dest" ]]; then
        printf 'file destination is a directory\n'
        return
    fi
    rendered="$(mktemp)" || { printf 'cannot create resolver preview\n'; return; }
...
resolver_apply() {  # <resolver> <repo-source> <live-destination>
    local resolver="$1" fn
    resolver_method "$resolver" apply; fn="$RESOLVER_METHOD"
    shift
    if declare -F "$fn" >/dev/null 2>&1; then "$fn" "$@"
...
resolver_preview() {  # <resolver> <repo-source> <live-destination>
    local resolver="$1" src="$2" dest="$3" fn rendered
    resolver_method "$resolver" preview; fn="$RESOLVER_METHOD"
    if declare -F "$fn" >/dev/null 2>&1; then "$fn" "$src" "$dest"; return; fi
...
resolver_changes() {  # <resolver> <repo-source> <live-destination>
    local fn
    resolver_method "$1" changes; fn="$RESOLVER_METHOD"
    if declare -F "$fn" >/dev/null 2>&1; then shift; "$fn" "$@"
