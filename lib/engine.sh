...
replace_path_transaction() (  # <stage-root> <staged-path> <destination>
    ...
    if [[ "$had_old" == 1 ]]; then mv "$dest" "$old" || { rollback_path; return 1; }; fi
    mv "$staged" "$dest" || { rollback_path; return 1; }
    committed=1
...
sync_tool() {
    ...
    if mode_config_enabled && tool_has_config "$i" && ! tool_uptodate "$i"; then
        ...
        if [[ "$action" == link ]]; then active="linking"; result="linked"
        else active="copying"; result="copied"; fi
        if [[ -n "$run" ]]; then
